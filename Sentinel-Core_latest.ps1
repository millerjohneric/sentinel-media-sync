...
$RawSettings = $Config.Settings | ConvertFrom-Json
$Settings = if ($RawSettings.Settings) { $RawSettings.Settings } else { $RawSettings }
...
"@
    ...
    foreach ($Dir in $DirsToIndex) {
        ...
        $DirImgs  = Get-ChildItem $Dir.FullName -File | Where-Object { $_.Extension -match '\.(jpg|jpeg|png|png)$' -and $_.Name -notmatch '^_thumb_' } | Select-Object -First 12
...
function Global:Write-SentinelSidebars {
    ...
    # Generate sidebar for each module
    foreach ($M in $Modules) {
        $ModuleName = $M.Name
    ...
    # Also add a default sidebar for the main docs
            ...
            foreach ($Line in $Lines) {
                ...
                if (Test-Path $SourcePath) {
                    ...
                    foreach ($Side in $Sidecars) {
                        Write-Host "Processing: $($Side.FullName)" -ForegroundColor DarkGray
                        $Buddy = Get-SentinelBuddy -Sidecar $Side -SearchRoot $Loc.Path
...
if ($Settings.DisableSidecarReunion -ne $true -and $Settings.DisableSidecarReunion -ne "true") {
    Write-Host "  $($Global:Icons.Arrow) Reuniting orphaned sidecars..." -ForegroundColor Gray
    
    foreach ($ArchiveLoc in $ArchiveLocs) {
        if (!(Test-Path $ArchiveLoc.Path)) { continue }
    ...
    if (!$Settings.DisableJunkPurge -and $JunkList) {
        ...
        foreach ($Loc in $ArchiveLocs) {
            ...
            Get-ChildItem $Loc.Path -Recurse -File -ErrorAction SilentlyContinue |
                ...
                ForEach-Object {
                    if (!$DryRun) {
                        try { Remove-Item $_.FullName -Force -ErrorAction Stop; $JunkCount++; $Stats.Purged++ } catch {}
                    }
                }
        }
