...
function Global:Invoke-SentinelRecipeContent {
    ...
    # Ensure Target Directory exists so Docusaurus can find the files
    if (!(Test-Path $TargetDir)) { 
    ...
    $Files = Get-ChildItem $SourceDataDir -File
    
    if ($Files.Count -eq 0) {
        Write-Host "  $($Global:Icons.Warning) No source files found in $SourceDataDir" -ForegroundColor Yellow
        return
    }

    # Grouping by ID (the part before -.-)
    $GroupedItems = $Files | Group-Object { 
        if ($_.Name -match '-.-') { $_.BaseName.Split('-.-')[0] } else { $_.BaseName }
    }
    ...
    foreach ($Group in $GroupedItems) {
        ...
        # 1. Build Image and Asset Lists
        $ImageFiles = $Group.Group | Where-Object { $_.Extension -match 'jpg|png|svg|jpeg' }
        $VideoFiles = $Group.Group | Where-Object { $_.Extension -match 'mp4|mov|webm' }
        ...
        # 2. Map Placeholders
        # Handle the specific YAML key syntax for ingredients/images
        ...
        # Support for recipe-specific placeholders if using recipe-card.md
        $Content = $Content -replace '\{\{ingredients_list\}\}', $YamlImageList.TrimEnd() 
        $Content = $Content -replace '\{\{instructions_list\}\}', "  - 'Step 1: Processed via Sentinel Media Sync'"

        # 3. Write File
        $Content | Out-File $MarkdownFile -Encoding UTF8 -Force
        Write-Host "  $($Global:Icons.Check) Created: $($MarkdownFile)" -ForegroundColor Gray
    }
}
...
function Global:Write-SentinelSidebars {
    ...
    foreach ($loc in $Locations) {
        ...
        # Note: dirName is '.' because each plugin is rooted in its own folder
        $Content = @"
module.exports = {
  $($ID)Sidebar: [
    {
