            ...
            foreach ($Line in $Lines) {
                ...
                if (Test-Path $SourcePath) {
                    ...
                    foreach ($Side in $Sidecars) {
                        Write-Host "Processing: $($S.FullName)" -ForegroundColor DarkGray # Add this for visibility
    ...
    if (!$Settings.DisableJunkPurge) {
        ...
        foreach ($S in $Sidecars) {
            try {
                ...
                # Log the error with the specific file and message
                Write-Warning "Failed to process sidecar: $($S.FullName)"
