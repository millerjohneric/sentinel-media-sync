...
# 1. Static Assets
$ImgDir = Join-Path $Web "components\img"
if (-not (Test-Path $ImgDir)) { New-Item $ImgDir -ItemType Directory -Force }
Copy-Item "$Tmpl\favicon.ico" (Join-Path $Web "components\") -Force
...
Write-Host "Branding injected. Please run 'xSentinel Web Gen.ps1' to build your archive pages." -ForegroundColor Green