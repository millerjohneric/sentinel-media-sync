...
# --- PHASE 2: GENERATING WEBSITE CONTENT ---
...
foreach ($loc in $Global:YamlData.Locations) {
    if ($loc.Template) {
        ...
        # PASSING $Engine here ensures $Settings.TemplateDir is populated
        Invoke-SentinelWebPipeline -loc $loc -Settings $Engine -TargetWebsitePath $TargetWebsitePath
    }
}
...
# --- PHASE 3: FINALIZING STUDIO ---
Write-Host "`nPHASE 3: Finalizing Studio Framework..." -ForegroundColor Cyan
Invoke-SentinelBranding -SitePath $TargetWebsitePath -TemplateDir $Engine.TemplateDir
